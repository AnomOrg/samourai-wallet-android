            ...
            try {
                if(Bech32Util.getInstance().isBech32Script(scriptPubKey)) {
                    if(Bech32Util.getInstance().getAddressFromScript(scriptPubKey).compareToIgnoreCase(getDestAddress()) == 0) {
                        LogUtil.debug("SendParams", "send address identified:" + Bech32Util.getInstance().getAddressFromScript(scriptPubKey));
                    ...
                    LogUtil.debug("SendParams", "send address identified:" + _p2sh.toString());
                    LogUtil.debug("SendParams", "send address output index:" + i);
                    ret.add(i);
                }
                else if(_p2sh == null && _p2pkh != null && _p2pkh.toString().compareTo(getDestAddress()) == 0) {
                    LogUtil.debug("SendParams", "send address identified:" + _p2pkh.toString());
                    LogUtil.debug("SendParams", "send address output index:" + i);
                    ret.add(i);
                }
                else  {
