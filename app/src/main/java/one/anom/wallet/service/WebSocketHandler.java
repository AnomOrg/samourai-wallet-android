    ...
    public void send(String message) {
        ...
        try {
            if (mConnection != null && mConnection.isOpen()) {
                LogUtil.info("WebSocketHandler", "Websocket subscribe:" + message);
                ...
                mConnection = new WebSocketFactory()
                        ...
                        .addListener(new WebSocketAdapter() {
                            ...
                            public void onTextMessage(WebSocket websocket, String message) {
                                LogUtil.debug("WebSocket", message);
                                try {
                                        ...
                                        if (objX.has("out")) {
                                            ...
                                            for (int j = 0; j < outArray.length(); j++) {
                                                ...
                                                if((outObj.has("addr") && BIP47Meta.getInstance().getPCode4Addr(outObj.getString("addr")) != null) ||
                                                    ...
                                                    LogUtil.info("WebSocketHandler", "received from " + out_addr);
                                                    ...
                                                    if(pcode != null && idx > -1)    {
                                                        ...
                                                        for(int i = idx; i < (idx + BIP47Meta.INCOMING_LOOKAHEAD); i++)   {
                                                            LogUtil.info("WebSocketHandler", "receive from " + i + ":" + BIP47Util.getInstance(context).getReceivePubKey(new PaymentCode(pcode), i));
                                                    ...
                                                    LogUtil.info("WebSocketHandler", "addr:" + outObj.getString("addr"));
                ...
                for(int i = 0; i < inputs.length(); i++)   {
                    ...
                    if(inputObj.has("seq"))    {
                        ...
                        if(BigInteger.valueOf(sequence).compareTo(SamouraiWallet.RBF_SEQUENCE_VAL) <= 0)    {
                            ret = true;
                            LogUtil.debug("WebSocketHandler", "return value:" + ret);
                            break;
                        }
                    }
                }
