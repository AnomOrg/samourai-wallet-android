...
//            info("APIFactory", "idx:" + idx + " , " + pcode);
            ...
            for(int i = idx; i < (idx + INCOMING_LOOKAHEAD); i++)   {
                try {
                    LogUtil.info("APIFactory", "receive from " + i + ":" + BIP47Util.getInstance(ctx).getReceivePubKey(new PaymentCode(pcode), i));
        ...
        try {
            ...
            if(idxs != null)    {
                for(int i = 0; i < idxs.size(); i++)   {
                    LogUtil.info("BIP47Meta", "address has unspents:" + BIP47Util.getInstance(ctx).getReceivePubKey(new PaymentCode(pcode), idxs.get(i)));
