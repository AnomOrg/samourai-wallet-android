...
import one.anom.wallet.paynym.PayNymViewModel
import one.anom.wallet.send.*
...
class PayNymDetailsActivity : AnomActivity() {
        ...
        when (item.itemId) {
            ...
            R.id.send_menu_paynym_details -> {
                if (BIP47Meta.getInstance().getOutgoingStatus(pcode) == BIP47Meta.STATUS_SENT_CFM) {
                    val intent = Intent(this, one.anom.wallet.send.SendActivity::class.java)
    ...
    private fun doNotifTx() {
        ...
        scope.launch(Dispatchers.IO) {
            ...
            val httpClient: IHttpClient = AndroidHttpClient(one.anom.wallet.util.WebUtil.getInstance(applicationContext), torManager)
