...
public class APIFactory	{
    ...
    public synchronized boolean getToken(boolean setupDojo) {
        ...
        LogUtil.debug("APIFactory", "getToken() url:" + _url);
        ...
        try {
            ...
            if(AppUtil.getInstance(context).isOfflineMode())    {
                ...
                LogUtil.info("APIFactory", "API key url:" + _url);
            ...
            try {
                ...
                if(jsonObject != null && jsonObject.has("authorizations"))    {
                    ...
                    if(authObj.has("access_token"))    {
                        LogUtil.info("APIFactory", "setting access token:" + authObj.getString("access_token"));
    ...
    private synchronized JSONObject registerXPUB(String xpub, int purpose, String tag) {
        ...
        try {
            ...
            try {
                jsonObject = new JSONObject(response);
                LogUtil.info("APIFactory", "XPUB response:" + jsonObject.toString());
        ...
        if(jsonObject != null)  {
            ...
            if(jsonObject.has("wallet"))  {
                ...
                if(walletObj.has("final_balance"))  {
                    ...
                    LogUtil.debug("APIFactory", "xpub_balance:" + xpub_balance);
                }
            }
    ...
    private synchronized void bip47Lookahead(String pcode, String addr)  {
        LogUtil.debug("APIFactory", "bip47Lookahead():" + addr);
        LogUtil.debug("APIFactory", "bip47Lookahead():" + pcode);
        LogUtil.debug("APIFactory", "bip47Lookahead():" + BIP47Meta.getInstance().getPCode4Addr(addr));
        int idx = BIP47Meta.getInstance().getIdx4Addr(addr);
        LogUtil.debug("APIFactory", "bip47Lookahead():" + idx);
        try {
            idx++;
            for (int i = idx; i < (idx + BIP47Meta.INCOMING_LOOKAHEAD); i++) {
                LogUtil.info("APIFactory", "receive from " + i + ":" + BIP47Util.getInstance(context).getReceivePubKey(new PaymentCode(pcode), i));
...
//                        args.put("signature", Uri.encode(sig));
                        ...
                        args.put("at", getAccessToken());
                        LogUtil.info("APIFactory", "lock XPUB:" + _url);
                        LogUtil.info("APIFactory", "lock XPUB:" + args.toString());
                        response = WebUtil.getInstance(context).tor_postURL(_url + "xpub/" + xpub + "/lock/", args);
                        LogUtil.info("APIFactory", "lock XPUB response:" + response);
                    }
...
//            info("APIFactory", "Notif address:" + response);
    ...
    public void parseNotifTx(JSONObject jsonObject, String addr, String hash) throws JSONException  {
        ...
        LogUtil.info("APIFactory", "notif address:" + addr);
        LogUtil.info("APIFactory", "hash:" + hash);
        ...
        if(jsonObject != null)  {
            ...
            if(jsonObject.has("inputs"))    {
                ...
                if(inArray.length() > 0)    {
                    ...
                    LogUtil.info("APIFactory", "scriptsig:" + strScript);
                    if((strScript == null || strScript.length() == 0 || strScript.startsWith("160014")) && objInput.has("witness"))    {
                        ...
                        LogUtil.info("APIFactory", "pubkey from script:" + Hex.toHexString(script.getPubKey()));
                        pubkey = script.getPubKey();
                    }
                    ECKey pKey = new ECKey(null, pubkey, true);
                    LogUtil.info("APIFactory", "address from script:" + pKey.toAddress(SamouraiWallet.getInstance().getCurrentNetworkParams()).toString());
//                        info("APIFactory", "uncompressed public key from script:" + Hex.toHexString(pKey.decompress().getPubKey()));
                    ...
                    if(((JSONObject)inArray.get(0)).has("outpoint"))    {
                        ...
                        LogUtil.info("APIFactory", "outpoint:" + Hex.toHexString(outpoint));

                        try {
                            mask = BIP47Util.getInstance(context).getIncomingMask(pubkey, outpoint);
                            LogUtil.info("APIFactory", "mask:" + Hex.toHexString(mask));
                        }
                        catch(Exception e) {
                            e.printStackTrace();
                        }
            ...
            if(mask != null && payload != null)    {
                try {
                    ...
                    LogUtil.info("APIFactory", "xlat_payload:" + Hex.toHexString(xlat_payload));

                    pcode = new PaymentCode(xlat_payload);
                    LogUtil.info("APIFactory", "incoming payment code:" + pcode.toString());
            ...
            if(pcode != null)    {
                try {
                    ...
                    for(int i = 0; i < BIP47Meta.INCOMING_LOOKAHEAD; i++)   {
                        LogUtil.info("APIFactory", "receive from " + i + ":" + BIP47Util.getInstance(context).getReceivePubKey(pcode, i));
    ...
    public synchronized JSONObject getUnspentOutputs(String[] xpubs) {
        ...
        try {
            ...
            if(AppUtil.getInstance(context).isOfflineMode())    {
                ...
                args.append("active=");
                args.append(StringUtils.join(xpubs, URLEncoder.encode("|", "UTF-8")));
                LogUtil.debug("APIFactory", "UTXO args:" + args.toString());
                args.append("&at=");
                args.append(getAccessToken());
                response = WebUtil.getInstance(context).postURL(_url + "unspent?", args.toString());
                LogUtil.debug("APIFactory", "UTXO:" + response);
            }
            else    {
                HashMap<String,String> args = new HashMap<String,String>();
                args.put("active", StringUtils.join(xpubs, "|"));
            ...
            try {
                ...
                for (int i = 0; i < utxoArray.length(); i++) {
                    ...
                    try {
                        ...
                        if(outDict.has("xpub"))    {
                            ...
                            LogUtil.debug("APIFactory", outDict.getString("pubkey") + "," + pcode);
                            LogUtil.debug("APIFactory", outDict.getString("pubkey") + "," + idx);
                        }
                        else    {
                            ;
                        }
                ...
                for(String key : utxos.keySet())   {
                    for(MyTransactionOutPoint out : utxos.get(key).getOutpoints())    {
                        LogUtil.debug("APIFactory", "utxo:" + out.getAddress() + "," + out.getValue());
                        LogUtil.debug("APIFactory", "utxo:" + utxos.get(key).getPath());
                        amount += out.getValue().longValue();
                    }
                }
                LogUtil.debug("APIFactory", "utxos by value (post-parse):" + amount);
...
//                info("APIFactory", addressStrings.toString());
            ...
            LogUtil.debug("APIFactory", "addresses:" + addressStrings.toString());
...
//                    debug("APIFactory", "blocked removed:" + _s);
            ...
            LogUtil.debug("APIFactory", "pre-mix multi:" + preMultiAddrObj.toString(2));
            LogUtil.debug("APIFactory", "pre-mix unspent:" + preUnspentObj.toString());
            ...
            LogUtil.debug("APIFactory", "post-mix multi:" + postMultiAddrObj.toString());
            LogUtil.debug("APIFactory", "post-mix unspent:" + postUnspentObj.toString());
...
//            debug("APIFactory", "post-mix unspent:" + getUtxosPostMix().size());
            ...
            LogUtil.debug("APIFactory", "bad bank multi:" + badbankMultiAddrObj.toString());
            LogUtil.debug("APIFactory", "bad bank unspent:" + badbankUnspentObj.toString());
...
//                    debug("APIFactory", "not dusted removed:" + _s);
            ...
            */
            for(String _s : BlockedUTXO.getInstance().getBlockedUTXOPostMix().keySet())   {
                LogUtil.debug("APIFactory", "blocked post-mix:" + _s);
                if(!seenOutputsPostMix.contains(_s))    {
                    BlockedUTXO.getInstance().removePostMix(_s);
                    LogUtil.debug("APIFactory", "blocked removed:" + _s);
                }
            }
    ...
    public synchronized int syncBIP47Incoming(String[] addresses) {
        ...
        LogUtil.debug("APIFactory", "sync BIP47 incoming:" + jsonObject.toString());
        int ret = 0;
                ...
                for(int i = 0; i < addressArray.length(); i++)  {
                    ...
                    if(addrObj.has("address"))  {
                        ...
                        if(addrObj.has("final_balance"))  {
                            ...
                            if(amount > 0L)    {
                                BIP47Meta.getInstance().addUnspent(pcode, idx);
                                LogUtil.info("APIFactory", "BIP47 incoming amount:" + idx + ", " + addr + ", " + amount);
                            }
                            else    {
                                if(addrObj.has("pubkey"))    {
                                    String pubkey = addrObj.getString("pubkey");
                                    if(pubkeys.containsKey(pubkey))    {
                                        ...
                                        if(count == 3)    {
                                            BIP47Meta.getInstance().removeUnspent(pcode, Integer.valueOf(idx));
                                            LogUtil.info("APIFactory", "BIP47 remove unspent:" + pcode + ":" + idx);
                                        }
                                        else    {
                                            pubkeys.put(pubkey, count + 1);
                                        }
                        ...
                        if(addrObj.has("n_tx"))  {
                            ...
                            if(nbTx > 0)    {
                                ...
                                LogUtil.info("APIFactory", "sync receive idx:" + idx + ", " + addr);
                                ret++;
                            }
                        }
        ...
        try {
            ...
            if(jsonObject != null && jsonObject.has("addresses"))  {
                ...
                for(int i = 0; i < addressArray.length(); i++)  {
                    ...
                    String pcode = null;
                    int idx = -1;
                    LogUtil.info("APIFactory", "address object:" + addrObj.toString());
                    ...
                    if(addrObj.has("n_tx"))  {
                        ...
                        if(nbTx > 0)    {
                            if(idx >= BIP47Meta.getInstance().getOutgoingIdx(pcode))    {
                                LogUtil.info("APIFactory", "sync send idx:" + idx + ", " + addr);
...
//            }
        ...
        LogUtil.debug("APIFactory", "utxos by value:" + amount);
...
//            debug("APIFactory", "unspents found:" + outputsRoot.size());
                ...
                for (int i = 0; i < utxoArray.length(); i++) {
                    ...
                    try {
                        ...
                        if(Bech32Util.getInstance().isBech32Script(script))    {
                            address = Bech32Util.getInstance().getAddressFromScript(script);
                            LogUtil.debug("address parsed:", address);
                        }
                        else    {
                            address = new Script(scriptBytes).getToAddress(SamouraiWallet.getInstance().getCurrentNetworkParams()).toString();
                        }
