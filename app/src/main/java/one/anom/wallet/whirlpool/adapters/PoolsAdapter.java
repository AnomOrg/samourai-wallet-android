package one.anom.wallet.whirlpool.adapters;
...
import one.anom.wallet.whirlpool.models.PoolViewModel;
...
public class PoolsAdapter extends RecyclerView.Adapter<PoolsAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(final ViewHolder holder, int position) {
        ...
        holder.poolAmount.setText(FormatsUtil.getBTCDecimalFormat(poolViewModel.getDenomination(),2).concat(" BTC Pool"));
         holder.poolFees.setText(mContext.getString(R.string.pool_fee).concat(" ").concat(FormatsUtil.getBTCDecimalFormat(poolViewModel.getFeeValue())).concat(" BTC"));
        holder.totalFees.setText(mContext.getString(R.string.total_fees).concat("  ").concat(FormatsUtil.getBTCDecimalFormat(poolViewModel.getTotalFee())).concat(" BTC").concat(" (").concat(String.valueOf(poolViewModel.getTotalEstimatedBytes())).concat( " bytes)"));
        holder.minorFees.setText(mContext.getString(R.string.miner_fee).concat("  ").concat(FormatsUtil.getBTCDecimalFormat(poolViewModel.getMinerFee())).concat(" BTC"));
